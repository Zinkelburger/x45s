...
bool evaluateOffSuit(const Card& lhs, const Card& rhs, Suit::Suit suitLed) {
    ...
    if (lhs.getSuit() != suitLed && rhs.getSuit() != suitLed) {
        throw std::invalid_argument("Neither card is suit led!");
        return -1;
    }
...
bool lessThan(const Card& lhs, const Card& rhs, Suit::Suit suitLed, Suit::Suit trump) {
    ...
    int hearts[14] = {5, 11, 0xACE, 13, 12, 10, 9, 8, 7, 6, 4, 3, 2, -10};
    ...
    if (suitLed == Suit::HEARTS) {
        order = hearts;
    } else if (suitLed == Suit::DIAMONDS) {
        order = diamonds;
    } else {
        order = clubsAndSpades;
    }
    ...
    // this card is trump and other card is not
